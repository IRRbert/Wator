class classSzene {
    constructor() {
        ...
        this.groundTexture = this.textureLoader.load('https://threejs.org/examples/textures/terrain/grasslight-big.jpg');
        this.groundTexture.wrapS = this.groundTexture.wrapT = THREE.RepeatWrapping;
        this.groundTexture.repeat.set(100, 100);

        this.groundMaterial = new THREE.MeshStandardMaterial({ map: this.groundTexture });
        this.groundMaterial.side = THREE.DoubleSide;

        this.ground = new THREE.Mesh(new THREE.PlaneGeometry(3000, 3000), this.groundMaterial);
        this.ground.rotation.x = -Math.PI / 2;
        this.ground.receiveShadow = true;
        this.scene.add(this.ground);

        this.skyTexture = this.textureLoader.load('http://localhost/pic/w11973-small.jpg');
        this.skyMaterial = new THREE.MeshBasicMaterial({ map: this.skyTexture, side: THREE.BackSide });
        this.skySphere = new THREE.Mesh(new THREE.SphereGeometry(10000, 32, 32), this.skyMaterial);
        this.scene.add(this.skySphere);

        this.helperBoxGeometry = new THREE.BoxGeometry(100, 100, 100);
        this.helperBoxMaterial = new THREE.MeshBasicMaterial({ color: 0xff0000, wireframe: true });
        this.helperBox = new THREE.Mesh(this.helperBoxGeometry, this.helperBoxMaterial);
    ...
    animate() {
        ...
        // Rufen Sie die update-Methode für alle Objekte auf, die diese Methode haben
