...
class classWesen {
    ...
    static findRandomEmptyPosition_for_init(settings, fishList, sharkList) {
        const x = Math.floor(settings.dimensions.x / 2);
        const y = Math.floor(settings.dimensions.y / 2);
        const z = Math.floor(settings.dimensions.z / 2);
        for (let attempts = 0; attempts < 100; attempts++) {
            const pos = {
                x: Math.floor(Math.random() * x),
                y: Math.floor(Math.random() * y),
